...
class Fp_node:
    def __init__(self, value, count=1, parent=None):
        self.value = value
        self.count = count
        self.children = HashTable()  # utilisation du HashTable
        self.parent = parent
...
class FP_Tree:
    ...
    def visualize(self, filename="fp_tree"):
        """Generate a Graphviz diagram of the FP-tree"""
        dot = Digraph(comment="FP-Growth Tree", format="png")
        dot.node(str(id(self.root)), f"{self.root.value}\ncount={self.root.count}")

        def add_edges(node):
            for bucket in node.children.table:
                curr = bucket
                while curr:
                    child = curr.value
                    dot.node(str(id(child)), f"{child.value}\ncount={child.count}")
